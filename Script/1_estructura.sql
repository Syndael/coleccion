...
) ENGINE=InnoDB AUTO_INCREMENT=610 DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;

-- La exportación de datos fue deseleccionada.

-- Volcando estructura para tabla coleccion.BASE_DLC
DROP TABLE IF EXISTS `BASE_DLC`;
CREATE TABLE IF NOT EXISTS `BASE_DLC` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  `BASE_ID` int(11) NOT NULL,
  `NOMBRE` varchar(255) NOT NULL,
  PRIMARY KEY (`ID`) USING BTREE,
  KEY `BASE_ID_B_PK` (`BASE_ID`) USING BTREE,
  CONSTRAINT `FK_BASE_DLC_BASE` FOREIGN KEY (`BASE_ID`) REFERENCES `BASE` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;

-- La exportación de datos fue deseleccionada.

-- Volcando estructura para procedimiento coleccion.BASE_REFRESH_PLATAFORMAS
DROP PROCEDURE IF EXISTS `BASE_REFRESH_PLATAFORMAS`;
DELIMITER //
...
DROP TABLE IF EXISTS `BASE_X_PLATAFORMA`;
...
) ENGINE=InnoDB AUTO_INCREMENT=1517 DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
...
) ENGINE=InnoDB AUTO_INCREMENT=1235 DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
...
-- Volcando estructura para procedimiento coleccion.PROGRESO_REFRESH_SESION
DROP PROCEDURE IF EXISTS `PROGRESO_REFRESH_SESION`;
DELIMITER //
CREATE PROCEDURE `PROGRESO_REFRESH_SESION`(
	IN `PARAM_PROGRESO_ID` INT
)
BEGIN
	UPDATE PROGRESO SET
		HORAS = (SELECT SUM(HORAS) FROM PROGRESO_SESION WHERE PROGRESO_ID = PARAM_PROGRESO_ID),
		FECHA_ULTIMO = (SELECT MAX(FECHA_FIN) FROM PROGRESO_SESION WHERE PROGRESO_ID = PARAM_PROGRESO_ID GROUP BY PROGRESO_ID)
	WHERE ID = PARAM_PROGRESO_ID;
END//
DELIMITER ;

-- Volcando estructura para tabla coleccion.PROGRESO_SESION
DROP TABLE IF EXISTS `PROGRESO_SESION`;
CREATE TABLE IF NOT EXISTS `PROGRESO_SESION` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  `PROGRESO_ID` int(11) NOT NULL,
  `BASE_DLC_ID` int(11) DEFAULT NULL,
  `FECHA_INICIO` date DEFAULT NULL,
  `FECHA_FIN` date DEFAULT NULL,
  `HORAS` decimal(20,2) DEFAULT NULL,
  `FECHA_H_INICIO` date DEFAULT NULL,
  `FECHA_H_FIN` date DEFAULT NULL,
  `HORAS_H` decimal(20,2) DEFAULT NULL,
  `NOTAS` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`ID`) USING BTREE,
  KEY `PROG_ID_P_FK` (`PROGRESO_ID`) USING BTREE,
  KEY `BASE_DLC_ID_BD_FK` (`BASE_DLC_ID`) USING BTREE,
  CONSTRAINT `FK_PROGRESO_SESION_BASE_DLC` FOREIGN KEY (`BASE_DLC_ID`) REFERENCES `BASE_DLC` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `FK_PROGRESO_SESION_PROGRESO` FOREIGN KEY (`PROGRESO_ID`) REFERENCES `PROGRESO` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
DROP TABLE IF EXISTS `TIPO_BASE`;
...
DROP TABLE IF EXISTS `TIPO_FICHERO`;
...
DROP TRIGGER IF EXISTS `BASE_ON_INSERT`;
...
DROP TRIGGER IF EXISTS `BASE_ON_UPDATE`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
...
DROP TRIGGER IF EXISTS `BASE_X_PLATA_ON_DELETE`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
...
DROP TRIGGER IF EXISTS `BASE_X_PLATA_ON_INSERT`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
...
DROP TRIGGER IF EXISTS `COLECCION_ON_INSERT`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
...
DROP TRIGGER IF EXISTS `COLECCION_ON_UPDATE`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
		...
		CONCAT(
			IFNULL(
				(
					SELECT
						MIN(
							IF(e.DESCRIPCION = 'Terminado', 2, IF(e.DESCRIPCION = 'Jugando', 1, 0))
						)
					FROM PROGRESO p
...
DROP TRIGGER IF EXISTS `PROGRESO_ON_DELETE`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `PROGRESO_ON_DELETE` AFTER DELETE ON `PROGRESO` FOR EACH ROW BEGIN
	UPDATE COLECCION SET ACTUALIZACIONES = COALESCE(ACTUALIZACIONES, 0) + 1 WHERE ID = OLD.ID;
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Volcando estructura para disparador coleccion.PROGRESO_ON_INSERT
DROP TRIGGER IF EXISTS `PROGRESO_ON_INSERT`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
...
DROP TRIGGER IF EXISTS `PROGRESO_ON_UPDATE`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `PROGRESO_ON_UPDATE` BEFORE UPDATE ON `PROGRESO` FOR EACH ROW BEGIN
	UPDATE COLECCION SET ACTUALIZACIONES = COALESCE(ACTUALIZACIONES, 0) + 1 WHERE BASE_ID = NEW.BASE_ID;
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Volcando estructura para disparador coleccion.PROGRESO_SESION_ON_DELETE
DROP TRIGGER IF EXISTS `PROGRESO_SESION_ON_DELETE`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `PROGRESO_SESION_ON_DELETE` BEFORE DELETE ON `PROGRESO_SESION` FOR EACH ROW BEGIN
	CALL PROGRESO_REFRESH_SESION(OLD.PROGRESO_ID);
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Volcando estructura para disparador coleccion.PROGRESO_SESION_ON_INSERT
DROP TRIGGER IF EXISTS `PROGRESO_SESION_ON_INSERT`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `PROGRESO_SESION_ON_INSERT` AFTER INSERT ON `PROGRESO_SESION` FOR EACH ROW BEGIN
	CALL PROGRESO_REFRESH_SESION(NEW.PROGRESO_ID);
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Volcando estructura para disparador coleccion.PROGRESO_SESION_ON_UPDATE
DROP TRIGGER IF EXISTS `PROGRESO_SESION_ON_UPDATE`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `PROGRESO_SESION_ON_UPDATE` AFTER UPDATE ON `PROGRESO_SESION` FOR EACH ROW BEGIN
	CALL PROGRESO_REFRESH_SESION(NEW.PROGRESO_ID);
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;
...
DROP TRIGGER IF EXISTS `ROM_ON_DELETE`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
...
DROP TRIGGER IF EXISTS `ROM_ON_INSERT`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
...
DROP TRIGGER IF EXISTS `ROM_ON_UPDATE`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `COMPLETO` AS select row_number() over () AS `ID`,concat(case when month(`ps`.`FECHA_H_FIN`) = 1 then 'Enero' when month(`ps`.`FECHA_H_FIN`) = 2 then 'Febrero' when month(`ps`.`FECHA_H_FIN`) = 3 then 'Marzo' when month(`ps`.`FECHA_H_FIN`) = 4 then 'Abril' when month(`ps`.`FECHA_H_FIN`) = 5 then 'Mayo' when month(`ps`.`FECHA_H_FIN`) = 6 then 'Junio' when month(`ps`.`FECHA_H_FIN`) = 7 then 'Julio' when month(`ps`.`FECHA_H_FIN`) = 8 then 'Agosto' when month(`ps`.`FECHA_H_FIN`) = 9 then 'Septiembre' when month(`ps`.`FECHA_H_FIN`) = 10 then 'Octubre' when month(`ps`.`FECHA_H_FIN`) = 11 then 'Noviembre' when month(`ps`.`FECHA_H_FIN`) = 12 then 'Diciembre' end,' ',year(`ps`.`FECHA_H_FIN`)) AS `MES`,count(0) AS `CANTIDAD`,group_concat(case when `bd`.`ID` is not null then concat(`b`.`NOMBRE`,concat(': ',`bd`.`NOMBRE`)) when `bd`.`ID` is null then `b`.`NOMBRE` end order by `b`.`NOMBRE` ASC separator ', ') AS `JUEGOS`,date_format(`ps`.`FECHA_H_FIN`,'%Y%m') AS `ORDEN_DESC` from (((`PROGRESO_SESION` `ps` join `PROGRESO` `p` on(`ps`.`PROGRESO_ID` = `p`.`ID`)) join `BASE` `b` on(`p`.`BASE_ID` = `b`.`ID`)) left join `BASE_DLC` `bd` on(`ps`.`BASE_DLC_ID` = `bd`.`ID`)) where `ps`.`FECHA_H_FIN` is not null group by month(`ps`.`FECHA_H_FIN`),year(`ps`.`FECHA_H_FIN`) order by date_format(`ps`.`FECHA_H_FIN`,'%Y%m') desc;
